...
For Linux users it might be necessary to install the `latex` package `examdesign` and the system function `texi2dvi`. The following terminal code will make sure all `latex` requirements are installed and ready for use:
...
The use of RndTexExam is optimized when using a cloud based spreadsheet tool to register the answers for the students. This avoid the manual work usually needed to digitalize the information from the application of the exam. The package RndTexExam not only minimizes cheating but can also make the grading process painless. 
